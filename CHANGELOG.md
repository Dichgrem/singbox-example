...
- 新增 TCP 内核参数优化（Python 计算引擎，BDP 感知缓冲区，双场景自适应）

## v5.85.5

- Subhatch token 回退到查询字符串传参
- 自动更新开关拆分为独立的开/关菜单选项
- SSH 密钥设置检测已存在密钥时提前退出

## v5.85.0

- 提取共享辅助函数，修复 shellcheck 警告，强化安全
- 新增服务器初始化向导（apt 升级 + 基础包 + BBR 一键设置）
- 新增重装脚本安装/卸载功能
- 新增 SSH 密钥登录设置
- 修复 Trojan 协议名拼写错误（trajan → trojan）
- 拆分 sing-box 下载函数，跳过已是最新时的重启
